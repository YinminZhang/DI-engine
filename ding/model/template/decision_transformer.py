...
@MODEL_REGISTRY.register('multi_dt')
class MultiDecisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.embed_timestep = nn.Embedding(max_timestep, h_dim)
        self.embed_rtg = torch.nn.Linear(1, h_dim)
        self.embed_state = nn.ModuleList([torch.nn.Linear(sd, h_dim) for sd in state_dim])

        if self.continuous:
            action_tanh = True  # True for continuous actions
            self.embed_action = nn.ModuleList([torch.nn.Linear(ad, h_dim) for ad in act_dim])
        ...
        # prediction heads
        self.predict_rtg = torch.nn.Linear(h_dim, 1)
        self.predict_state = nn.ModuleList([torch.nn.Linear(h_dim, sd) for sd in state_dim])
        self.predict_action = nn.ModuleList([nn.Sequential(*([nn.Linear(h_dim, ad)] + ([nn.Tanh()] if action_tanh else []))) for ad in act_dim])
    ...
    def forward(
        ...
        # time embeddings are treated similar to positional embeddings
        # shape: (B,context_len,h_dim)
        state_embeddings = self.embed_state[self.state_dim.index(C)](states) + time_embeddings
        action_embeddings = self.embed_action[self.state_dim.index(C)](actions) + time_embeddings
        returns_embeddings = self.embed_rtg(returns_to_go) + time_embeddings
        ...
        # get predictions
        return_preds = self.predict_rtg(h[..., 2, :])  # predict next rtg given r, s, a
        state_preds = self.predict_state[self.state_dim.index(C)](h[..., 2, :])  # predict next state given r, s, a
        action_preds = self.predict_action[self.state_dim.index(C)](h[..., 1, :])  # predict action given r, s
