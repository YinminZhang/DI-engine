...
from ding.rl_utils import discount_cumsum, discount_cumsum_past
...
class D4RLTrajectoryDataset(Dataset):
    ...
    def __init__(self, cfg: dict) -> None:
        ...
        # cfg.policy.learn.dataset_path, cfg.policy.context_len, cfg.policy.rtg_scale
        # dataset_path: str, context_len: int, rtg_scale: float
        
        dataset_path = cfg.policy.learn.dataset_path
        ...
        self.discrete = cfg.policy.get('discrete', False)
        self.discrete_bin = cfg.policy.get('discrete_bin', 0)
        ...
        for traj in self.trajectories:
            ...
            if cfg.policy.learn.get('with_decrement_return', True):
                # calculate returns to go and rescale them
                traj['returns_to_go'] = discount_cumsum(traj['rewards'], 1.0) / rtg_scale
            else:
                rtg = discount_cumsum(traj['rewards'], 1.0) / rtg_scale
                rtg = np.ones_like(rtg)*rtg[0]
                traj['returns_to_go'] = rtg
            if self.discrete:
                traj['returns_to_go'] = traj['returns_to_go'] * self.discrete_bin * rtg_scale // self.cfg.policy.rtg_target
            if self.cfg.policy.model.get('state_goal', False):
                traj['returns_to_go'] = discount_cumsum(traj['observations'], 1.0) / len(traj['observations'])
            if self.cfg.policy.get('use_past_reward', False):
                traj['returns_to_go'] = discount_cumsum_past(traj['rewards'], 1.0) / rtg_scale
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if traj_len >= self.context_len:
            ...
            si = np.random.randint(0, traj_len - self.context_len + 1)
...
@DATASET_REGISTRY.register('d4rl_list_trajectory')
class D4RLTrajectoryListDataset(Dataset):
