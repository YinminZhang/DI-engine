...
@POLICY_REGISTRY.register('qrdqn_bc')
class QRDQNBCPolicy(DQNPolicy):
    r"""
    Overview:
    ...
    def _forward_learn(self, data: dict) -> Dict[str, Any]:
        ...
        # behavior cloning
        replay_q = q_value.mean(-1)
        bc_loss = F.cross_entropy(replay_q, data['action'])
        lmbda = self._alpha / q_value.mean(-1).abs().mean().detach()
        loss += lmbda * bc_loss
